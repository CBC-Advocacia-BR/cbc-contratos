...
🚫 **DECISÃO DO PAULO (03/08): o contrato FICA COM OS EMOJIS.** Levei os trechos do contrato real lado a lado com duas alternativas e ele escolheu manter. ⚠️ O motivo técnico que valia para as telas **não vale ali**: emoji ignora a cor do texto, mas o contrato é sempre branco (papel ou PDF); o que sobra é o desenho variar por aparelho, e pesado contra deixar o resumo mais leve para o cliente ler, ele preferiu a leveza. A decisão está escrita no topo do próprio `contractHtml.js`, que é onde uma futura varredura vai olhar. Também não tocados: os emojis do contrato (`contractHtml.js`) — é o documento que o cliente assina, tem 6 snapshots travando o texto, e mudá-lo é decisão de negócio, não faxina. Também ficaram: comentários de código, o ChangeLog (registro histórico) e a mensagem de boas-vindas do portal, que vai por WhatsApp **ao cliente**.
